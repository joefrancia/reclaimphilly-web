...
urlpatterns = patterns('',
    ...
    url(r'^services/put/location/latitude/(?P<latitude>[+-]?\d{1,2}(\.\d{1,13})?)/longitude/(?P<longitude>[+-]?\d{1,3}(\.\d{1,13})?)/type/(?P<type>\w{1,4})(/address/(?P<address>(\w|.|\s)+))?(/description/(?P<description>(\w|\s|[.-])))?$', 'reclaimphilly_web.rest_services.add_location'), # This will take GET arguments for description, address, and picture (the optional params)
)

...
urlpatterns = patterns('',
    ...
    url(r'^services/get/locations/latitude/(?P<latitude>[+-]?\d{1,2}(\.\d{1,13})?)/longitude/(?P<longitude>[+-]?\d{1,3}(\.\d{1,13})?)/radius/(?P<radius>[+-]?\d{1,5}(\.\d{1,13})?)', 'reclaimphilly_web.rest_services.get_locations'),
